...
#                       is now the current file
#                   - 5.11.2025: Updated script to find and pull the most recent schema file instead
#                       of a specific file name. ADDITIONALLY:
#                             If you want to restore from a particular backup file rather than the most recent one:
#
#                                 1. Open your terminal or command prompt
#                                 2. Navigate to the directory containing the RestoreCaseysSchema.py script
#                                 3. Run the script with the specific backup file as an argument:
#                                     ***BASH COMMAND***
#                                       python RestoreCaseysSchema.py path/to/your/specific_backup_file.sql
#
#                                       For example:
#                                            ***BASH COMMAND***
#                                            python RestoreCaseysSchema.py mydatabase_schema_backup_20250410_142355.sql
#
#                                4. The script will verify that the specified file exists. If it doesn't exist,
#                                   it will automatically fall back to using the most recent backup file.

#                   - 05.11.2025: Updated to remove user input response in terminal to allow DB_BigRedButton.py to
#                       function correctly
#
#
# **********************************************************************************************************************
...
generated by the schema backup script. It automatically finds the
most recent backup file if none is specified.
"""
...
def find_latest_backup_file():
    """
    Find the most recent schema backup file created by GetCaseysSchema.py
    """
    # Default pattern for backup files (assuming they follow the pattern from GetCaseysSchema.py)
    backup_pattern = "*_schema_backup_*.sql"

    # Get all matching files in the current directory
    backup_files = glob.glob(backup_pattern)

    if not backup_files:
        print(f"Error: No schema backup files found matching pattern '{backup_pattern}'")
        print("Please ensure GetCaseysSchema.py has been run or specify a backup file manually.")
        sys.exit(1)
    ...
    print(f"Found latest backup file: {latest_file} (created on {mod_time_str})")
    return latest_file
...
def restore_database_schema(backup_file_path=None, auto_yes=False):
    """
    Main function to restore a database from a schema backup file
    """
    # If no backup file specified, find the most recent one
    if not backup_file_path:
        backup_file_path = find_latest_backup_file()
    elif not os.path.exists(backup_file_path):
        print(f"Error: Specified backup file {backup_file_path} does not exist")
        print("Attempting to find the most recent backup file instead...")
        backup_file_path = find_latest_backup_file()
    ...
    try:
        ...
        if db_name in databases:
            ...
            # If auto_yes is True, automatically drop and recreate without prompting
            if auto_yes:
                print(f"Auto-confirm is enabled. Dropping and recreating database '{db_name}'...")
...
if __name__ == "__main__":
    ...
    parser.add_argument('backup_file', nargs='?',
                        help='Path to the schema backup SQL file (if not specified, the most recent backup file will be used)')
    parser.add_argument('--yes', '-y', action='store_true',
                        help='Automatically answer "yes" to all prompts (use with caution)')
    args = parser.parse_args()

    print("=== MariaDB Schema Restore Tool ===")
    db_name = restore_database_schema(args.backup_file, args.yes)
