...
-- Generated on: 2025-05-26 22:52:10
...
-- Structure for table `employee_department_history`
...
CREATE TABLE `employee_department_history` (
  ...
  CONSTRAINT `fk_emp_dept_hist_department` FOREIGN KEY (`DPTID`) REFERENCES `department` (`DPTID`) ON UPDATE CASCADE,
  CONSTRAINT `fk_emp_dept_hist_employee` FOREIGN KEY (`EMPID`) REFERENCES `employee_table` (`EMPID`) ON UPDATE CASCADE
...
-- Structure for table `employee_role_history`
DROP TABLE IF EXISTS `employee_role_history`;
CREATE TABLE `employee_role_history` (
  `ROLE_HISTORY_ID` int(11) NOT NULL AUTO_INCREMENT,
  `EMPID` varchar(20) NOT NULL,
  `EMP_ROLE` varchar(20) NOT NULL,
  `ROLE_ASSIGNMENT_DATE` date NOT NULL,
  `CREATED_TIMESTAMP` datetime DEFAULT current_timestamp(),
  PRIMARY KEY (`ROLE_HISTORY_ID`),
  KEY `idx_role_empid` (`EMPID`),
  KEY `idx_role_assignment_date` (`ROLE_ASSIGNMENT_DATE`),
  KEY `idx_emp_role` (`EMP_ROLE`),
  CONSTRAINT `fk_emp_role_hist_employee` FOREIGN KEY (`EMPID`) REFERENCES `employee_table` (`EMPID`) ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=41 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `time` (
  ...
  `FLAGGED_FOR_REVIEW` tinyint(1) DEFAULT 0 COMMENT 'Flag indicating if time entry needs manager review',
  PRIMARY KEY (`TIMEID`),
  KEY `EMPID` (`EMPID`),
  KEY `START_TIME` (`START_TIME`),
  KEY `STOP_TIME` (`STOP_TIME`),
  KEY `fk_time_new_project` (`PROJECTID`),
  KEY `idx_flagged_for_review` (`FLAGGED_FOR_REVIEW`),
...
-- Table `employee_department_history`: ~40 rows, Data: 0.02MB, Indexes: 0.05MB
-- Table `employee_projects`: ~210 rows, Data: 0.02MB, Indexes: 0.03MB
-- Table `employee_role_history`: ~40 rows, Data: 0.02MB, Indexes: 0.05MB
-- Table `employee_table`: ~40 rows, Data: 0.02MB, Indexes: 0.05MB
-- Table `login_table`: ~40 rows, Data: 0.02MB, Indexes: 0.02MB
-- Table `projects`: ~91 rows, Data: 0.02MB, Indexes: 0.03MB
-- Table `time`: ~541 rows, Data: 0.09MB, Indexes: 0.08MB
...
-- FK: employee_department_history.DPTID -> department.DPTID (fk_emp_dept_hist_department)
-- FK: employee_department_history.EMPID -> employee_table.EMPID (fk_emp_dept_hist_employee)
...
-- FK: employee_role_history.EMPID -> employee_table.EMPID (fk_emp_role_hist_employee)
...
-- INDEX: employee_department_history.idx_assignment_date (ASSIGNMENT_DATE)
-- INDEX: employee_department_history.idx_dptid (DPTID)
-- INDEX: employee_department_history.idx_empid (EMPID)
-- INDEX: employee_projects.EMPID (EMPID)
-- INDEX: employee_projects.PROJECT_ID (PROJECT_ID)
-- INDEX: employee_role_history.idx_emp_role (EMP_ROLE)
-- INDEX: employee_role_history.idx_role_assignment_date (ROLE_ASSIGNMENT_DATE)
-- INDEX: employee_role_history.idx_role_empid (EMPID)
...
-- INDEX: time.idx_flagged_for_review (FLAGGED_FOR_REVIEW)
